...
** DESCRIPTION: Responds with all transactions of a user, along with the net outgoing and incoming amounts
...
** 		"incoming": {
**			{
**				"fromUserPhoneNumber": phoneNUmber,
**				"amount": amount,
...
function query_user_transaction($query_json) {
	$_POST = $query_json;
	$response = array();
	
	require_once 'db_connect.php';
	
	$outgoing_transaction_sql = "SELECT toUserPhoneNumber, amount, transactionGroupsetID FROM transaction WHERE fromUserPhoneNumber=" . $_POST['userPhoneNumber'];
	$incoming_transaction_sql = "SELECT fromUserPhoneNumber, amount, transactionGroupsetID FROM transaction WHERE toUserPhoneNumber=" . $_POST['userPhoneNumber'];
